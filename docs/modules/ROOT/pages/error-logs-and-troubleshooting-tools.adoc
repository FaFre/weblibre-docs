...
Proxy and Tor runtime messages have a separate live viewer under **Settings > Proxy > Proxy Connections > View logs**.
Use **Proxy Logs** when a connection will not start or reports as connected but pages do not load.
You can filter by level, copy or share the visible output, and clear the log.
