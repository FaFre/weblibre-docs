...
If the shortcut uses Tor or a proxy, WebLibre starts that connection automatically and waits before loading the page, without asking first. If the connection cannot start, the shortcut does not fall back to a direct connection. See xref:proxy.adoc[Proxy Connections].

Login, payment, and other flows that use `window.open` or popup windows stay within the PWA flow instead of being lost behind the standalone window.

== Fullscreen Video and Picture-in-Picture

Fullscreen video in a WebLibre web app can enter Android Picture-in-Picture when you press Home or switch apps, then return to the same PWA when you come back.
