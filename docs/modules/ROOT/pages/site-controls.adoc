...
* decide whether links from that site can open another app
...
== App Links For One Site

Under **App Links** in the site panel, choose **Follow default**, **Open in app**, or **Keep in browser**. Use this when you want one site to behave differently from **Settings > Browsing > Open Links in Apps**. You can remove saved site rules from **Remembered site rules** in those browsing settings.

Opening a link in another app can expose your activity to that app. In a private tab, a Tor- or proxy-routed tab, or a strict container, WebLibre asks before opening it regardless of an automatic-open setting. For the full set of choices, see xref:custom-tabs-and-external-links.adoc[Custom Tabs & External Links].
...
You can also enable **Close tab after clearing** if you want the current tab closed once the data is gone.
If you leave it off, WebLibre reloads the current tab after clearing.

Then tap **Clear Now** and confirm the action.

WebLibre detaches open sessions for the same site before wiping data so they cannot immediately recreate it.
...
For a tab in a cookie-isolated container, Gecko cannot combine host-specific clearing with that container context. WebLibre therefore clears the selected data across that container, not only for the current host.
====
...
If a site that relies on third-party redirects for legitimate functionality stops working, you can adjust the behavior from the *Safe Browsing* group in xref:privacy/web-engine-hardening.adoc[Web Engine Hardening] or from the **Redirect Trackers** toggle under *Custom* tracking protection.
